...
mask = src[:,:,-1] # 가로세로 그대로 가져오기, -1 : 맨 마지막 채널 사용하기
src = src[:,:,0:3] # 가로세로 ~, 0:3  0,1,2채널 사용하기
dst = cv2.imread('field.bmp' ,cv2.IMREAD_COLOR)

#서로 채널이 다르기때문에  crop을 해줘야한다
h, w =src.shape[:2] # 처음 두개가 h, w임
crop =dst[0:h, 0:w ] # 픽셀값을 공유 ? 
#crop =dst[10:h+10, 10:w+10 ] # 픽셀값을 공유 ? 



#src영상에서 비행기 부분(mask)만 dst영상에 합성하고 싶음
cv2.copyTo(src,mask,crop) # src영상에서 mask부분을 추출해서 dst에 합성
...
cv2.imshow('src',src) # 3채널짜리
cv2.imshow('dst',dst) # 1채널 짜리
